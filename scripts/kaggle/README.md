...
> `--logdir` dùng path tuyệt đối (`/kaggle/working/calculus-of-learning/outputs/...`),
> **không** dùng `../../outputs/...` tương đối — thư viện `elements` (vendor trong
> DreamerV3) có bug tự cắt bớt một cấp `..` ở đầu path tương đối (`../../foo` →
> `./../foo`), gây `FileNotFoundError` khi ghi `config.yaml`. Path tuyệt đối
> không bị bug này.
...
> nhưng host TPU **không có GPU**, EGL không tìm được device, MuJoCo rơi về GLFW,
> đụng `DISPLAY` không tồn tại và **segfault**. Set trong cell Python (`os.environ`)
> là không đủ chắc chắn; export thẳng trong cell bash mới ăn chắc.

```bash
%%bash
export MUJOCO_GL=osmesa      # T4x2/RTX 6000: egl
export JAX_PLATFORMS=tpu     # T4x2/RTX 6000: bỏ dòng này
cd /kaggle/working/calculus-of-learning/third_party/dreamerv3
timeout 900 python dreamerv3/main.py \
  --logdir /kaggle/working/calculus-of-learning/outputs/dreamer/dmc_walker_walk/seed0 \
...
> `--run.envs 8`: preset `dmc_proprio` kế thừa mặc định `run.envs: 16`, tức 16
> process MuJoCo song song cho **một** run. Khi chạy 5 seed song song ở Cell 4b
> thì thành 80 process trên cùng một host — quá tải CPU và dễ segfault. Giảm còn
> 8 (hoặc 4 nếu vẫn crash). Kiểm số core bằng `!nproc` trước khi quyết định.
...
OUT=/kaggle/working/calculus-of-learning/outputs/dreamer
mkdir -p $OUT
cd /kaggle/working/calculus-of-learning/third_party/dreamerv3
...
for SEED in 0 1 2 3 4; do
  ...
  ( TPU_VISIBLE_CHIPS=$CHIP python dreamerv3/main.py \
      ...
      --seed ${SEED} --run.steps 5e5 --run.envs 8 --jax.platform tpu \
      > $OUT/${TASK}_seed${SEED}.log 2>&1 & )
done
wait
```
...
for SEED in 0 1 2 3 4; do
  ...
  ( CUDA_VISIBLE_DEVICES=$GPU python dreamerv3/main.py \
      --logdir $OUT/${TASK}/seed${SEED} \
      --configs dmc_proprio --task ${TASK} \
      --seed ${SEED} --run.steps 5e5 --run.envs 8 --jax.platform cuda \
      > $OUT/${TASK}_seed${SEED}.log 2>&1 & )
